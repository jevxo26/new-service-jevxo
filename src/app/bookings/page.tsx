...
export default function BookingsPage() {
  ...
  if (isAuthLoading) {
    return (
      <div className="min-h-screen bg-[#E6F0FA]/10 flex flex-col justify-center items-center py-16">
  ...
  if (!isAuthenticated) {
    return (
      <section className="min-h-screen bg-gradient-to-b from-[#E6F0FA]/40 via-white to-white py-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden flex items-center">
        ...
        <div className="mx-auto max-w-2xl w-full text-center relative z-10 space-y-6">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 120 }} className="inline-flex p-4.5 bg-[#E6F0FA] text-[#1E4E8C] rounded-3xl border border-[#1E4E8C]/15 shadow-sm shadow-[#1E4E8C]/5 mb-2">
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="show" className="mt-8 grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 text-left">
            <motion.div variants={cardFadeUp} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:border-[#1E4E8C]/10 transition-colors">
              <div className="w-8 h-8 bg-[#E6F0FA] text-[#1E4E8C] rounded-xl flex items-center justify-center mb-3"><Clock className="w-4 h-4" /></div>
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-[#E6F0FA]/20 via-white to-white px-4 py-8 md:py-12 lg:px-8 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-5xl relative z-10">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-6 mb-8">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-[#E6F0FA] text-[#1E4E8C] rounded-2xl border border-[#1E4E8C]/15 shadow-sm shadow-[#1E4E8C]/2">
        ...
        <motion.div layout className="space-y-4">
          {isBookingsLoading ? (
            ...
            <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="py-20 text-center bg-white rounded-3xl border border-slate-100 shadow-sm space-y-4">
              <div className="w-12 h-12 rounded-full bg-[#E6F0FA] text-[#1E4E8C] flex items-center justify-center mx-auto border border-[#1E4E8C]/10"><FileText className="w-6 h-6" /></div>
                  ...
                  <motion.div key={booking.id} layout initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ type: "spring", stiffness: 100, damping: 15 }} whileHover={{ y: -3, boxShadow: "0 12px 30px -5px rgba(0,0,0,0.03)" }} className={`flex flex-col md:flex-row md:items-center justify-between gap-4 rounded-2xl border-l-4 bg-white p-5 border border-slate-100/90 transition-all ${cfg.borderColor} ${cfg.glowColor}`}>
                    ...
                    <div className="flex flex-row md:flex-col md:items-end justify-between items-center gap-3 pt-3.5 md:pt-0 border-t border-slate-50 md:border-0">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => setSelectedBooking(booking)} className="rounded-xl border border-slate-200 hover:border-[#1E4E8C] hover:text-[#1E4E8C] bg-slate-50/50 hover:bg-[#E6F0FA] px-4 py-2.5 text-xs font-extrabold text-slate-600 transition-all hover:scale-105 active:scale-95 cursor-pointer flex items-center gap-1">
