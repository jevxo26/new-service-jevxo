...
export default function BookingFilters({ search, setSearch, activeFilter, setActiveFilter }: BookingFiltersProps) {
  return (
    <div className="bg-white p-5 rounded-3xl border border-slate-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.015)] mb-6 space-y-4">
      <div className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50/50 px-4 py-3 focus-within:border-[#1E4E8C]/30 focus-within:bg-white focus-within:ring-4 focus-within:ring-[#1E4E8C]/5 transition-all">
          ...
          <button
            ...
            className={`rounded-xl px-4 py-2.5 text-xs font-extrabold transition-all cursor-pointer ${
              activeFilter === f.value
                ? "bg-[#1E4E8C] text-white shadow-sm shadow-[#1E4E8C]/20"
                : "bg-[#E6F0FA] text-[#1E4E8C] hover:bg-[#FFF0EB] border border-[#1E4E8C]/10"
            }`}
          >
            {f.label}
          </button>
