  ...
  return (
    <div className="bg-transparent overflow-hidden flex-1 flex flex-col relative">
      ...
      <section ref={heroRef} className="relative pt-8 pb-6 md:pt-12 md:pb-10 lg:pt-16 overflow-hidden">
        <motion.div style={{ y: glowY }} className="pointer-events-none absolute -top-20 right-[-10%] w-[600px] h-[600px] rounded-full bg-[#1E4E8C]/5 blur-[130px]" />
        ...
        <div className="pointer-events-none absolute top-0 right-0 w-72 h-72 border-l border-b border-[#1E4E8C]/6 rounded-bl-full" />
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center relative z-10">
          <RevealSection variants={fadeLeft}>
            <div className="inline-flex items-center gap-2 text-[10px] font-extrabold text-[#1E4E8C] uppercase tracking-[.12em] bg-[#E6F0FA] px-3.5 py-1.5 rounded-full border border-[#1E4E8C]/20 mb-5">
              <span className="w-1.5 h-1.5 rounded-full bg-[#1E4E8C] animate-pulse" />Our Story — Since 2023
            </div>
          ...
          <RevealSection variants={stagger}>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {displayStats.map((stat, i) => (
                <motion.div key={i} variants={fadeUp} className="relative group bg-white rounded-3xl border border-slate-100 p-6 text-center shadow-[0_4px_20px_rgba(0,0,0,0.02)] hover:border-[#1E4E8C]/20 hover:shadow-[0_10px_40px_rgba(30,78,140,0.05)] transition-all duration-300">
                  <p className="text-3xl md:text-4xl font-black text-[#1E4E8C] leading-none tracking-tight">{stat.value}</p>
      ...
      <section className="py-6 md:py-10 bg-transparent">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <RevealSection className="text-center mb-10">
            <span className="inline-flex items-center gap-1.5 text-[10px] font-extrabold text-[#1E4E8C] uppercase tracking-[.12em] bg-[#E6F0FA] px-3.5 py-1.5 rounded-full border border-[#1E4E8C]/20 mb-4"><Target className="w-3.5 h-3.5" />The Jevxo Services Standard</span>
          ...
          <RevealSection variants={stagger}>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
              {PILLARS.map(({ title, description, icon: Icon, bg }, i) => (
                <motion.div key={i} variants={fadeUp} className="group bg-white rounded-3xl border border-slate-100 p-6 hover:border-[#1E4E8C]/15 hover:shadow-[0_12px_40px_rgba(30,78,140,0.04)] transition-all duration-300">
      ...
      <section className="py-6 md:py-10 bg-transparent">
        <div className="max-w-7xl mx-auto px-4 md:px-6 grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          <RevealSection variants={fadeLeft}>
            <span className="inline-flex items-center gap-1.5 text-[10px] font-extrabold text-[#1E4E8C] uppercase tracking-[.12em] bg-[#E6F0FA] px-3.5 py-1.5 rounded-full border border-[#1E4E8C]/20 mb-5"><Users className="w-3 h-3" />Our Mission</span>
            ...
            <div className="space-y-3.5">
              {["Reducing unemployment for skilled trades in urban areas", "Creating a transparent, tech-driven service economy", "Ensuring safety and quality standards in every household", "Building a trusted community for clients and vendors alike"].map((item, i) => (
                <div key={i} className="flex items-start gap-3 text-[12px] font-semibold text-slate-600">
                  <div className="w-5 h-5 bg-[#E6F0FA] border border-[#1E4E8C]/20 rounded-full flex items-center justify-center shrink-0 mt-0.5"><CheckCircle2 className="w-3 h-3 text-[#1E4E8C]" /></div>
                  {item}
                </div>
              ))}
            </div>
          ...
          <RevealSection variants={fadeRight}>
            <div className="bg-white rounded-3xl border border-slate-100 p-7 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
              <div className="flex items-center gap-2 mb-5"><div className="p-2 rounded-xl bg-[#E6F0FA]"><MapPin className="w-4 h-4 text-[#1E4E8C]" /></div><h3 className="font-black text-sm text-slate-800 uppercase tracking-wider">Services We Cover</h3></div>
              <div className="space-y-3">
                {SERVICES_COVERED.map((service, i) => (
                  <div key={i} className="flex items-center gap-3 text-[12px] font-semibold text-slate-600 py-2.5 border-b border-slate-50 last:border-0">
                    <div className="w-5 h-5 bg-[#E6F0FA] rounded-lg flex items-center justify-center shrink-0 border border-[#1E4E8C]/15"><Check className="w-3 h-3 text-[#1E4E8C]" /></div>
                    {service}
                  </div>
                ))}
              </div>
      ...
      <section className="py-6 md:py-10 bg-transparent">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <RevealSection className="text-center mb-10">
            <span className="inline-flex items-center gap-1.5 text-[10px] font-extrabold text-[#1E4E8C] uppercase tracking-[.12em] bg-[#E6F0FA] px-3.5 py-1.5 rounded-full border border-[#1E4E8C]/20 mb-4"><Users className="w-3.5 h-3.5" />Our Leadership</span>
          ...
          <RevealSection variants={stagger}>
            <div className="grid md:grid-cols-3 gap-5">
              {teamMembers.map((member, i) => (
                <motion.div key={i} variants={fadeUp} className="group bg-white rounded-3xl border border-slate-100 overflow-hidden hover:border-[#1E4E8C]/15 hover:shadow-[0_12px_40px_rgba(30,78,140,0.05)] transition-all duration-300">
                  ...
                  <div className="p-5">
                    <p className="text-[9px] font-extrabold text-[#1E4E8C] uppercase tracking-[.12em] mb-1">{member.role}</p>
      ...
      <section className="pt-6 pb-4 md:pt-8 md:pb-6 bg-transparent border-t border-slate-100/60">
        <div className="max-w-4xl mx-auto px-4 md:px-6 text-center">
          <RevealSection>
            <span className="inline-flex items-center gap-1.5 text-[10px] font-extrabold text-[#1E4E8C] uppercase tracking-[.12em] bg-[#E6F0FA] px-3.5 py-1.5 rounded-full border border-[#1E4E8C]/20 mb-5"><Sparkles className="w-3.5 h-3.5" />Ready to Get Started?</span>
