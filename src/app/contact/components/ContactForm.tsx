        ...
        {!submitted ? (
          <motion.form key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleSubmit} noValidate className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              {[{ id: "name", label: "Full Name", type: "text", placeholder: "Mahbubur Rahman", required: true }, { id: "email", label: "Email Address", type: "email", placeholder: "yourname@gmail.com", required: true }].map(({ id, label, type, placeholder, required }) => (
                <div key={id} className="space-y-1.5">
                  <label htmlFor={id} className="text-[10px] font-black text-slate-400 uppercase tracking-[.1em]">{label} {required && <span className="text-[#1E4E8C]">*</span>}</label>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label htmlFor="subject" className="text-[10px] font-black text-slate-400 uppercase tracking-[.1em]">Subject Topic <span className="text-[#1E4E8C]">*</span></label>
            ...
            <div className="space-y-1.5">
              <label htmlFor="message" className="text-[10px] font-black text-slate-400 uppercase tracking-[.1em]">Details & Context <span className="text-[#1E4E8C]">*</span></label>
            ...
            <Button type="submit" disabled={isLoading} className="w-full bg-[#1E4E8C] hover:bg-[#e84e53] disabled:opacity-60 text-white text-xs font-extrabold py-3.5 h-auto rounded-xl border-none transition-colors shadow-sm flex items-center justify-center gap-2">
