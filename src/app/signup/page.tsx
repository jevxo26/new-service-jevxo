...
import {
  Mail, User, Check, ShieldCheck, Loader2, ArrowRight, Sparkles, ChevronLeft, UserPlus, Lock, Phone,
} from "lucide-react"
...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen w-full flex bg-white font-sans">
      {/* LEFT PANEL */}
      <div className="hidden lg:flex lg:w-[48%] xl:w-[50%] flex-col items-center justify-center relative overflow-hidden bg-gradient-to-br from-blue-50 via-indigo-50 to-slate-50">
        <div className="absolute top-[-80px] left-[-80px] w-72 h-72 bg-[#1E4E8C]/10 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute bottom-[-60px] right-[-60px] w-64 h-64 bg-blue-300/10 rounded-full blur-3xl pointer-events-none" />
        <div className="relative z-10 flex flex-col items-center text-center px-10">
          ...
          <p className="text-slate-500 text-sm font-medium mt-3 max-w-sm leading-relaxed">
            Join thousands of happy customers who trust Jevxo for professional services every day.
          </p>
      ...
      <div className="flex-1 flex flex-col justify-center overflow-y-auto relative">
          ...
          <div className="w-full max-w-[460px] bg-white/70 sm:bg-transparent backdrop-blur-sm sm:backdrop-blur-none border border-slate-200/80 sm:border-0 rounded-3xl shadow-lg sm:shadow-none px-6 py-8 sm:px-2 sm:py-0">
            <div className="flex flex-col items-center text-center mb-8">
              <Link href="/" className="flex flex-col items-center gap-2.5 group mb-5">
                <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-[#1E4E8C] to-[#5B92E5] rounded-2xl flex items-center justify-center text-white shadow-xl shadow-[#1E4E8C]/25 group-hover:scale-105 transition-transform">
              ...
              <p className="text-slate-500 text-sm font-medium mt-2 leading-relaxed max-w-xs">Sign up to access premium services.</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-widest">Full Name</label>
                <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"><User size={17} /></div>
                  <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="John Doe" className="w-full pl-11 pr-4 py-3.5 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:border-[#1E4E8C] focus:ring-4 focus:ring-[#1E4E8C]/10 focus:outline-none transition-all text-sm font-medium text-slate-900 placeholder-slate-400" required />
                </div>
              </div>

              <div className="space-y-1.5">
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-widest">Email Address</label>
                <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"><Mail size={17} /></div>
                  <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="name@company.com" className="w-full pl-11 pr-4 py-3.5 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:border-[#1E4E8C] focus:ring-4 focus:ring-[#1E4E8C]/10 focus:outline-none transition-all text-sm font-medium text-slate-900 placeholder-slate-400" required />
                </div>
              </div>

              <div className="space-y-1.5">
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-widest">Password</label>
                <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"><Lock size={17} /></div>
                  <input type="password" name="password" value={formData.password} onChange={handleChange} placeholder="At least 6 characters" className="w-full pl-11 pr-4 py-3.5 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:border-[#1E4E8C] focus:ring-4 focus:ring-[#1E4E8C]/10 focus:outline-none transition-all text-sm font-medium text-slate-900 placeholder-slate-400" required minLength={6} />
                </div>
              </div>

              <div className="space-y-1.5">
                <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-widest">Phone Number (Optional)</label>
                <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"><Phone size={17} /></div>
                  <input type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="01712XXXXXX" className="w-full pl-11 pr-4 py-3.5 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:border-[#1E4E8C] focus:ring-4 focus:ring-[#1E4E8C]/10 focus:outline-none transition-all text-sm font-medium text-slate-900 placeholder-slate-400" />
                </div>
              </div>
              ...
              <button type="submit" disabled={isLoading} className="w-full mt-1 bg-[#1E4E8C] hover:bg-[#123C73] disabled:opacity-70 text-white text-[15px] font-bold py-4 rounded-xl shadow-lg shadow-[#1E4E8C]/25 transition-all flex items-center justify-center gap-2 active:scale-[0.98] cursor-pointer border-none">
        ...
        <div className="text-center py-5 text-[11px] text-slate-400 font-medium relative z-10 border-t border-slate-100 mx-6 space-y-1">
          <div>© {new Date().getFullYear()} Jevxo Services · All rights reserved. <Link href="/privacy" className="ml-3 text-slate-400 hover:text-[#1E4E8C] transition-colors">Privacy</Link><Link href="/terms" className="ml-3 text-slate-400 hover:text-[#1E4E8C] transition-colors">Terms</Link></div>
        </div>
      </div>
    </div>
  )
