...
export default function CategoryDetailClientPage({ slug }: { slug: string }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FFFDFD] via-slate-50/70 to-[#E6F0FA] relative pb-16 overflow-x-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="sticky top-[72px] z-30 max-w-7xl mx-auto bg-[#E6F0FA]/50 backdrop-blur-lg border-b border-slate-100 shadow-[0_4px_24px_-6px_rgba(0,0,0,0.02)] hidden md:block">
          <div className="max-w-7xl mx-auto px-4 md:px-6 py-2 flex gap-2">
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => state.scrollToSection(tab.id)} className={`text-xs font-bold px-4 py-2.5 rounded-full transition-all duration-300 cursor-pointer ${state.activeTab === tab.id ? "text-white bg-[#1E4E8C] shadow-md shadow-rose-100" : "text-slate-400 hover:text-slate-700 hover:bg-slate-50"}`}>{tab.label}</button>
            ))}
          </div>
        </div>
