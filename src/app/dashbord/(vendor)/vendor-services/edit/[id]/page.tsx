...
import { useUpdateServiceMutation, useGetServiceByIdQuery } from "@/redux/features/admin/service";
...
export default function EditVendorServicePage() {
  ...
  const { data: serviceDataRes, isLoading: isServiceLoading } = useGetServiceByIdQuery(serviceId, { skip: !serviceId });
  ...
  useEffect(() => {
    if (serviceDataRes?.data) {
      const item = serviceDataRes.data;
      setName(item.name || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("সার্ভিস সফলভাবে আপডেট হয়েছে!");
      router.push("/dashbord/vendor-services");
    } catch (err: any) {
      toast.error(err.data?.message || err.message || "সার্ভিস আপডেট করতে ব্যর্থ হয়েছে");
    }
  };
