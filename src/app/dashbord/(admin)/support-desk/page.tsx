...
function StatCard({ label, count, accent, icon: Icon }: { label: string; count: number; accent: string; icon: React.ComponentType<any> }) {
  return (
    <div className="bg-white rounded-3xl border border-slate-100 shadow-xs p-6 flex items-center justify-between hover:shadow-md hover:border-[#1E4E8C]/15 transition-all duration-300 group">
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-200">
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 rounded-[32px] border border-slate-800/80 px-8 py-6 shadow-xl">
        <div className="absolute -top-12 -right-12 w-48 h-48 bg-[#1E4E8C]/10 rounded-full blur-3xl pointer-events-none" />
        
        <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-1.5 bg-white/5 border border-white/10 text-[#1E4E8C] text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full">
          ...
          >
            <RefreshCw size={13} className="text-[#1E4E8C]" />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label={lang === "bn" ? "নতুন পেন্ডিং" : "New Pending"} count={stats.pending} accent="bg-gradient-to-br from-[#1E4E8C] to-[#123C73] shadow-[#1E4E8C]/20" icon={AlertCircle} />
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-5 space-y-4 shadow-xs">
            <div className="flex items-center justify-between border-b border-slate-50 pb-2">
              <span className="text-[11px] font-black text-slate-700 uppercase tracking-widest flex items-center gap-1.5">
                <Filter size={12} className="text-[#1E4E8C]" />
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-700 placeholder:text-slate-400 outline-none focus:border-[#1E4E8C] focus:bg-white transition-colors"
            />
            ...
            <div className="grid grid-cols-2 gap-2">
              <select
                ...
                className="bg-slate-50 border border-slate-100 rounded-xl px-2.5 py-2.5 text-xs font-bold text-slate-600 outline-none focus:border-[#1E4E8C] bg-white cursor-pointer"
              >
          ...
          {selectedTicketId ? (
            <div className="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden flex flex-col min-h-[550px] border-slate-150/60">
              ...
              {activeTicketData?.data?.user && (
                <div className="px-5 py-3.5 border-b border-slate-100 bg-[#E6F0FA]/10 flex flex-wrap gap-4 text-[10px] font-bold text-slate-500">
                  <span className="flex items-center gap-1.5"><User size={13} className="text-[#1E4E8C]" /> {activeTicketData.data.user.name}</span>
                  <span className="flex items-center gap-1.5"><Mail size={13} className="text-[#1E4E8C]" /> {activeTicketData.data.user.email}</span>
                  {activeTicketData.data.user.phone && (
                    <span className="flex items-center gap-1.5"><Phone size={13} className="text-[#1E4E8C]" /> {activeTicketData.data.user.phone}</span>
                  )}
                </div>
              )}
            ...
            <div className="bg-white border border-slate-100 rounded-3xl p-16 text-center space-y-3 min-h-[550px] flex flex-col justify-center items-center">
              <div className="p-4 bg-slate-50 rounded-full border border-slate-100 text-[#1E4E8C]/65">
