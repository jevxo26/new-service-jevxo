    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-3 duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-slate-900">{lang === "bn" ? "সুপার অ্যাডমিন ম্যানেজমেন্ট" : "Super Admin Management"}</h1>
            <p className="text-xs text-slate-400 mt-0.5">{lang === "bn" ? "সুপার অ্যাডমিনদের দেখুন এবং পরিচালনা করুন।" : "Manage super administrators."}</p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          >
            {lang === "bn" ? "সুপার অ্যাডমিন যোগ করুন" : "Add Super Admin"}
          </button>
        </div>
      </div>
      ...
      {isUsersLoading ? (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-12 text-center">
          ...
          <p className="text-sm text-slate-400 font-medium">{lang === "bn" ? "লোড হচ্ছে..." : "Loading..."}</p>
        </div>
      ) : users.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-12 text-center">
          <p className="text-sm text-slate-400 font-medium">{lang === "bn" ? "কোনো সুপার অ্যাডমিন পাওয়া যায়নি।" : "No super admins found."}</p>
        </div>
      ) : (
        <UserTable
          users={users}
