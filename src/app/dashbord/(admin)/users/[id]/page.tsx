  ...
  if (isUserLoading || isBookingsLoading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="w-10 h-10 border-4 border-[#1E4E8C] border-t-transparent rounded-full animate-spin"></div>
      </div>
    );
  }
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm">
            <div className="flex flex-col items-center text-center">
              <div className="w-24 h-24 bg-gradient-to-br from-rose-100 to-rose-50 text-[#1E4E8C] rounded-full flex items-center justify-center mb-4 text-3xl font-black shadow-inner">
              ...
              <span className={`mt-2 inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${
                user.status === "active" ? "bg-emerald-50 text-emerald-700" : "bg-[#E6F0FA] text-[#123C73]"
              }`}>
                {user.status || "Unknown"}
              </span>
            </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                <Activity size={20} className="text-[#1E4E8C]" /> Recent Bookings
              </h3>
            </div>
                  ...
                  <div key={booking.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl border border-slate-100 hover:border-slate-200 hover:shadow-sm bg-slate-50/50 transition-all gap-4">
                    ...
                    <div className="flex flex-col sm:items-end gap-2">
                      ...
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${
                        ...
                        booking.status === 'cancelled' ? 'bg-[#FFF0EB] text-[#123C73]' :
