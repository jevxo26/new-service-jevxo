...
export default function ManualInvoicePage() {
  ...
  const fetchInvoices = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem("jevxo_access_token") || localStorage.getItem("token") || "";
  ...
  return (
    <div className="w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-6 print:hidden">
        ...
        <Link href="/dashbord/manual-invoice/create" className="flex items-center gap-2 bg-[#1E4E8C] text-white hover:bg-[#e0530a] font-bold px-4 py-2.5 rounded-xl text-sm transition-all shadow-sm cursor-pointer active:scale-[0.98]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 print:hidden">
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-100/80 shadow-sm transition-all hover:translate-y-[-3px] hover:border-[#1E4E8C]/20 hover:shadow-md duration-300 relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start pl-1">
            ...
            <div className="p-3 bg-[#E6F0FA] text-[#1E4E8C] rounded-2xl border border-[#1E4E8C]/15 shadow-xs transition-colors group-hover:bg-[#1E4E8C] group-hover:text-white">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 border-b border-slate-100 bg-slate-50/50">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {(["all", "Paid", "Due"] as const).map((f) => (
              <button
                key={f}
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border border-slate-205 cursor-pointer hover:bg-[#E6F0FA] hover:text-[#1E4E8C] ${
                  filter === f
                    ? "bg-[#1E4E8C] text-white border-[#1E4E8C] hover:bg-[#e0530a] hover:text-white"
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center gap-2 justify-end">
                        <Link
                          ...
                          className="flex items-center gap-1.5 bg-slate-50 border border-slate-200/60 hover:bg-[#E6F0FA] hover:text-[#1E4E8C] text-slate-600 font-bold px-3 py-1.5 rounded-xl transition-all shadow-xs cursor-pointer active:scale-95"
                        >
                          <Eye size={14} /> View
                        </Link>
                        <button
