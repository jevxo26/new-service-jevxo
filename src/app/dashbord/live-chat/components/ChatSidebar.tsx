      ...
      <div className="p-4 border-b border-slate-100 bg-white space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
            <MessageSquare className="w-4 h-4 text-[#1E4E8C]" />
            {lang === "bn" ? "ইনবক্স চ্যাট" : "Inbox Chat"}
          </h2>
          <span className="text-[10px] font-extrabold text-[#1E4E8C] bg-[#E6F0FA] border border-[#1E4E8C]/20 px-2 py-0.5 rounded-full">
            {lang === "bn" ? `মোট ${filteredInbox.length}` : `Total ${filteredInbox.length}`}
          </span>
        </div>

        {/* Search bar inside inbox list */}
        <div className="flex items-center gap-2 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2 focus-within:border-[#1E4E8C]/40 focus-within:bg-white focus-within:ring-2 focus-within:ring-[#1E4E8C]/5 transition-all">
              ...
              return (
                <motion.button
                  ...
                  className={`w-full text-left p-3 flex items-center gap-3 transition-all cursor-pointer rounded-2xl ${
                    isActive
                      ? "bg-[#E6F0FA] border-l-4 border-l-[#1E4E8C] border border-[#1E4E8C]/15 shadow-sm shadow-[#1E4E8C]/5"
                ...
                >
                    ...
                    {item.user?.avatar || item.user?.image ? (
                      ...
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-xs ${
                          isActive ? "bg-gradient-to-br from-[#1E4E8C] to-[#FF8142] text-white" : "bg-slate-100 text-slate-500"
                        }`}
                      >
                        {item.user?.name ? (
                  ...
                  <div className="flex-1 overflow-hidden space-y-0.5">
                    ...
                    <p className={`text-[11px] truncate ${isActive ? "text-[#123C73] font-semibold" : "text-slate-400 font-medium"}`}>
                      {item.lastMessage}
                    </p>
                  </div>
                </motion.button>
