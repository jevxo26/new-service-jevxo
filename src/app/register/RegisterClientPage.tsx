...
import { REGISTER_CONTENT, containerVariants, itemVariants } from "@/app/register/components/RegisterComponents";
...
export default function RegisterClientPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-[#FAF8F8] flex items-center justify-center py-12 px-4 md:px-6 overflow-hidden">
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-7xl w-full grid md:grid-cols-2 gap-12 lg:gap-16 items-center">
        
        {/* Left Side - Hero Section */}
        <div className="hidden md:flex flex-col justify-center space-y-8 pl-4 lg:pl-8">
          <motion.div variants={itemVariants}>
            <div className="inline-flex items-center gap-2 bg-[#FF6014] text-white px-4 py-1.5 rounded-full text-xs font-bold tracking-wider mb-6">
              {REGISTER_CONTENT.badge}
            </div>
            <h1 className="text-4xl lg:text-5xl font-extrabold text-slate-900 leading-tight tracking-tight">
              {REGISTER_CONTENT.heroTitle} <span className="text-[#FF6014] block lg:inline">{REGISTER_CONTENT.heroAccent}</span>
            </h1>
            ...
            {REGISTER_CONTENT.benefits.map((benefit, i) => {
              ...
              return (
                <motion.div key={i} variants={itemVariants} className="flex gap-4 items-start">
                  <div className="w-12 h-12 rounded-2xl bg-white border border-slate-100 flex items-center justify-center flex-shrink-0 text-[#FF6014] shadow-sm">
                    <Icon className="w-5 h-5" />
                  </div>
                  <div>
                    <h4 className="font-extrabold text-slate-900 text-base">{benefit.title}</h4>
                    <p className="text-sm text-slate-500 mt-0.5">{benefit.description}</p>
                  </div>
                </motion.div>
              );
            })}
          ...
          <motion.div variants={itemVariants} className="mt-4 rounded-3xl overflow-hidden shadow-[0_15px_30px_rgba(0,0,0,0.03)] h-44 bg-gradient-to-r from-slate-200 to-slate-400/80 opacity-90 relative">
        ...
        <motion.div variants={itemVariants} className="w-full max-w-lg mx-auto">
          <div className="bg-white rounded-3xl shadow-[0_15px_40px_rgba(0,0,0,0.02)] border border-slate-100 p-8 md:p-12 relative overflow-hidden">
            <div className="mb-8">
              <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">{REGISTER_CONTENT.formTitle}</h2>
              <p className="text-slate-500 mt-2 text-sm">{REGISTER_CONTENT.formSubtitle}</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <Button type="submit" className="w-full bg-[#FF6014] hover:bg-[#E0530A] text-white font-bold py-4 h-auto rounded-xl transition-all duration-200 flex items-center justify-center gap-2 shadow-sm hover:shadow-md active:scale-[0.985] text-base mt-6 cursor-pointer">
            ...
            <div className="mt-8 text-center text-xs md:text-sm">
              <p className="text-slate-500 leading-relaxed">
                ...
                <Link href="#" className="text-[#FF6014] font-bold hover:underline">{REGISTER_CONTENT.termsLink}</Link> and <Link href="#" className="text-[#FF6014] font-bold hover:underline">{REGISTER_CONTENT.privacyLink}</Link>.
              </p>
              <p className="mt-4 text-slate-550 font-semibold">
                {REGISTER_CONTENT.alreadyHaveText}
                <Link href="/login" className="text-[#FF6014] font-extrabold hover:underline">{REGISTER_CONTENT.loginLink}</Link>
              </p>
            </div>
          </div>
        </motion.div>
