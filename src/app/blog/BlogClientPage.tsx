...
function BlogDetail({ blog, onClose }: { blog: any; onClose: () => void }) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-3 duration-300 space-y-6 sm:space-y-8">
      ...
      <button
        onClick={onClose}
        className="inline-flex items-center gap-1.5 text-xs font-extrabold text-slate-400 hover:text-[#1E4E8C] transition-colors"
      >
      ...
      <div className="space-y-3 max-w-3xl">
        {blog.createdAt && (
          <div className="flex items-center gap-1.5 text-[10px] font-black text-slate-400 uppercase tracking-widest">
            <Calendar size={10} className="text-[#1E4E8C]" />
        ...
        {blog.overview && (
          <p className="text-sm font-semibold text-slate-600 leading-relaxed border-l-4 border-[#1E4E8C]/50 pl-4 py-2 bg-[#E6F0FA] rounded-r-2xl">
            {blog.overview}
          </p>
        )}
      </div>
                ...
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
                  {imgs.map((_, i) => (
                    <button
                      ...
                      className={`rounded-full transition-all ${i === activeImg ? "bg-[#1E4E8C] w-5 h-2" : "bg-white/60 hover:bg-white w-2 h-2"
                        }`}
                    />
                  ))}
                </div>
              ...
              {imgs.map((img, i) => (
                <button
                  ...
                  className={`relative w-20 h-14 sm:w-24 sm:h-16 rounded-xl overflow-hidden border-2 transition-all shrink-0 ${i === activeImg
                      ? "border-[#1E4E8C] shadow-md shadow-orange-100 opacity-100"
      ...
      <div className="flex flex-col md:grid md:grid-cols-12 gap-6 md:gap-8">
        ...
        <div className="md:col-span-8 space-y-4">
          <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest pb-3 border-b border-slate-100">
            <BookOpen size={11} className="text-[#1E4E8C]" /> Full Article
          </div>
        ...
        <div className="md:col-span-4">
          <div className="bg-gradient-to-br from-[#FFF9F6] to-[#FFF1E9] border border-orange-100 rounded-2xl sm:rounded-3xl p-5 sm:p-6 space-y-4">
            <h3 className="font-black text-slate-800 text-sm flex items-center gap-1.5 uppercase tracking-wider">
              <ShieldCheck size={14} className="text-[#1E4E8C]" /> Need Help?
            </h3>
            ...
            <div className="flex flex-row sm:flex-col gap-2">
              <a
                href="/services"
                className="flex-1 flex items-center justify-center bg-[#1E4E8C] hover:bg-[#123C73] text-white text-xs font-bold py-2.5 px-4 rounded-xl transition-all shadow-sm shadow-orange-500/10"
              >
                Book Service
              </a>
              <a
                href="tel:01813333373"
                className="flex-1 flex items-center justify-center bg-white border border-orange-100 hover:bg-orange-50/50 text-[#1E4E8C] text-xs font-bold py-2.5 px-4 rounded-xl transition-all"
              >
                Call Hotline
              </a>
            </div>
...
function BlogCard({ blog, index, onClick }: { blog: any; index: number; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="group w-full text-left bg-white/80 backdrop-blur-md rounded-2xl sm:rounded-3xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-xl hover:shadow-orange-50/60 hover:border-[#1E4E8C]/20 transition-all duration-300 flex flex-col"
    >
      {/* Image */}
      <div className="relative w-full h-44 sm:h-52 bg-[#E6F0FA] overflow-hidden shrink-0">
        {thumb ? (
          <Image
            src={thumb}
            alt={blog.title}
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-2">
            <ImageOff className="w-7 h-7 text-[#1E4E8C]/30" />
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-1">
        {blog.createdAt && (
          <div className="flex items-center gap-1 text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2.5">
            <Calendar size={9} className="text-[#1E4E8C]" />
            {formatDate(blog.createdAt)}
          </div>
        )}
        ...
        <h2 className="font-bold text-[15px] sm:text-[17px] leading-snug mb-2.5 line-clamp-2 tracking-tight">
          <span className="text-[#1E4E8C]">{firstWord} </span>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-50 mt-auto">
          <span className="text-[9px] sm:text-[10px] font-extrabold text-[#1E4E8C] uppercase tracking-wider bg-[#E6F0FA] px-2.5 py-1 rounded-full">
            Read Article
          </span>
          <ChevronRight size={15} className="text-slate-200 group-hover:text-[#1E4E8C] group-hover:translate-x-1 transition-all" />
        </div>
      </div>
    </button>
  );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-[#E6F0FA] text-[#1E4E8C] rounded-xl shrink-0">
              ...
              <div>
                ...
                <h1 className="text-lg sm:text-xl font-extrabold text-slate-900 leading-tight">
                  Expert Tips <span className="text-[#1E4E8C]">& Home Care</span> Insights
                </h1>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 md:mt-10 relative z-10 w-full">
        ...
        {selectedBlog ? (
          ...
          <div className="text-center py-24 sm:py-32 bg-white/70 backdrop-blur-md border border-dashed border-slate-200 rounded-2xl sm:rounded-3xl space-y-3">
            <div className="w-14 h-14 bg-[#E6F0FA] text-[#1E4E8C] rounded-2xl flex items-center justify-center mx-auto">
              <BookOpen size={24} />
            </div>
