  ...
  return (
    ...
    >
      <div className="relative h-48 overflow-hidden">
        ...
        <span className="absolute top-3 right-3 flex items-center gap-1 py-1 px-2 bg-white/95 backdrop-blur-sm rounded-lg text-[10px] font-extrabold text-[#1E4E8C] shadow-sm border border-slate-100/50">
          <Star size={11} className="fill-[#1E4E8C] text-[#1E4E8C]" strokeWidth={0} />
          {service.rating}
        </span>

        {/* Bookings Completed Badge */}
        <div className="absolute bottom-3 left-3 bg-[#1E4E8C]/90 backdrop-blur-md text-white px-2.5 py-1 rounded-full text-[9px] font-black flex items-center gap-1.5 shadow-[0_4px_12px_rgba(30,78,140,0.25)] border border-white/20 uppercase tracking-wider z-10">
        ...
        <button
          ...
          className={`absolute bottom-3 right-3 w-8 h-8 rounded-full flex items-center justify-center shadow-md transition-all duration-200 cursor-pointer disabled:opacity-60 z-20 ${isWishlisted
              ? "bg-[#1E4E8C] text-white scale-110"
              : "bg-white/90 text-slate-400 hover:bg-rose-50 hover:text-[#1E4E8C] hover:scale-110"
            }`}
        >
          <Heart
            size={14}
      ...
      <div className="p-5 flex flex-col flex-1 justify-between">
        <div>
          <h3 className="text-sm font-black text-slate-800 mb-1.5 leading-snug group-hover:text-[#1E4E8C] transition-colors duration-200">
            {service.title}
          </h3>
        ...
        <div className="flex items-center justify-between mt-5 pt-4 border-t border-slate-50">
          <div className="flex flex-col">
            <span className="text-base font-black text-[#1E4E8C]">
          ...
          <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-50 text-[10px] font-extrabold text-[#1E4E8C] group-hover:bg-[#1E4E8C] group-hover:text-white transition-all duration-300 border border-slate-100 group-hover:border-[#1E4E8C]">
            Details
