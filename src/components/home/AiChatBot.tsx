      ...
      if (isInternal) {
        parts.push(
          <Link
            ...
            className="inline-flex items-center gap-1 bg-[#1E4E8C]/10 hover:bg-[#1E4E8C]/20 text-[#1E4E8C] px-2.5 py-1 rounded-full text-[10px] font-extrabold transition-all border border-[#1E4E8C]/20 hover:scale-[1.03] my-0.5 ml-1"
          >
            {label}
          </Link>
        );
    ...
    <>
      ...
      <div className="fixed bottom-[84px] right-4 md:bottom-6 md:right-6 z-[999] flex flex-col items-center gap-3">
        ...
        <div className="relative flex items-center justify-center group">
          ...
          <motion.span
            ...
            className="absolute w-9 h-9 md:w-14 md:h-14 rounded-full bg-gradient-to-r from-[#1E4E8C] to-[#FF7C71] blur-md pointer-events-none z-[-1]"
          />
          ...
          <motion.button
            whileHover={{
              scale: 1.1,
              rotate: -8,
              boxShadow: "0 14px 32px rgba(30, 78, 140, 0.45)"
            }}
            whileTap={{ scale: 0.92 }}
            onClick={() => setIsOpen(!isOpen)}
            className="w-9 h-9 md:w-14 md:h-14 bg-gradient-to-r from-[#1E4E8C] to-[#FF7C71] text-white rounded-full flex items-center justify-center shadow-lg shadow-[#1E4E8C]/25 cursor-pointer relative overflow-hidden transition-all duration-300"
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-[#1E4E8C] to-[#FF7C71] p-4 text-white flex items-center justify-between shadow-sm">
            ...
            {/* Message Area */}
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#E6F0FA]/30 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-slate-200">
              {messages.map((msg, i) => (
                <div
                  key={i}
            ...
            <div className="p-3 border-t border-slate-100 bg-white">
              <form
                ...
                className="flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-2xl p-1.5 pr-2 focus-within:bg-white focus-within:border-[#1E4E8C]/30 focus-within:ring-4 focus-within:ring-[#E6F0FA] transition-all"
              >
                <input
                  type="text"
                  value={inputValue}
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-xl bg-[#1E4E8C] hover:bg-[#123C73] text-white flex items-center justify-center transition disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shrink-0"
                >
                  <Send size={14} />
                </button>
              </form>
