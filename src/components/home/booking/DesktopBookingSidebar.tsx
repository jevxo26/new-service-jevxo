...
export function DesktopBookingSidebar({
    ...
    return (
      <div className="bg-white border border-slate-100 rounded-[32px] p-6 shadow-[0_8px_30px_rgba(0,0,0,0.02)] space-y-6">
        <div className="flex items-center gap-2 pb-3 border-b border-slate-100">
          <ShoppingCart size={18} className="text-[#1E4E8C]" />
          <h3 className="font-black text-slate-900 text-sm md:text-base">Booking Summary</h3>
        </div>
        <div className="bg-[#E6F0FA] border border-[#1E4E8C]/10 rounded-2xl p-4 text-center">
          <p className="text-xs font-bold text-slate-500">Select services from the list to start booking.</p>
        </div>
        <div className="space-y-4">
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#1E4E8C]">Why Choose Jevxo</p>
          <div className="space-y-3.5">
            {trustPoints.map(({ icon: Icon, text }) => (
              <div key={text} className="flex items-center gap-3">
                <div className="w-9 h-9 bg-[#E6F0FA] rounded-xl flex items-center justify-center shrink-0 border border-[#1E4E8C]/10">
                  <Icon className="w-4 h-4 text-[#1E4E8C]" />
                </div>
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-[32px] p-6 shadow-xl space-y-5 max-h-[calc(100vh-170px)] overflow-y-auto [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-rose-200 [&::-webkit-scrollbar-thumb]:rounded-full">
      ...
      <div className="bg-[#E6F0FA] rounded-2xl p-4 border border-[#1E4E8C]/10 space-y-2 text-xs">
        ...
        <div className="flex justify-between items-center text-sm font-black text-slate-800 pt-2 border-t border-slate-200/50">
          ...
          <span className="text-[#1E4E8C] text-base">৳{payableTotal.toLocaleString()}</span>
        </div>
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-4 pt-2">
        ...
        <button type="submit" disabled={isBooking} className="w-full py-3.5 mt-2 bg-[#1E4E8C] hover:bg-[#123C73] text-white font-extrabold rounded-2xl text-sm transition-all shadow-md shadow-rose-100 hover:shadow-lg cursor-pointer flex items-center justify-center gap-2">
