...
export function SubServiceDetailCard({
  ...
  return (
    <div className="bg-white rounded-[32px] border border-slate-100/80 shadow-[0_12px_40px_rgba(0,0,0,0.03)] overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
      {/* Header Banner */}
      <div className="bg-[#E6F0FA] px-6 py-5 border-b border-[#1E4E8C]/10 flex items-center justify-between">
        <div className="min-w-0">
          <span className="text-[10px] text-[#1E4E8C] font-black uppercase tracking-widest block mb-1">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between bg-slate-50/60 p-4 rounded-2xl border border-slate-100/50">
          <div>
            ...
            <div className="text-[#1E4E8C] text-2xl font-black">
              ৳{Number(subService.price).toLocaleString()}
            </div>
          </div>
          <div className="w-32">
            {isAdded ? (
              <div className="flex items-center gap-1 bg-white border border-[#1E4E8C]/20 rounded-xl p-0.5 w-full justify-between shadow-xs">
                <button
                  type="button"
                  onClick={() => onUpdateQuantity(subService.id, -1)}
                  className="w-8 h-8 rounded-lg bg-slate-50 text-[#1E4E8C] flex items-center justify-center hover:bg-rose-50/50 transition cursor-pointer"
                >
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 cursor-pointer active:scale-95 bg-[#1E4E8C] text-white hover:bg-[#123C73] shadow-md shadow-rose-100"
              >
        ...
        {faqs.length > 0 && (
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
              <HelpCircle size={14} className="text-[#1E4E8C]" />
              Service FAQs
            </h4>
            <div className="space-y-2.5">
          ...
          >
            ...
            <div className="px-6 pb-4 border-b border-slate-100 flex items-center justify-between shrink-0">
              <div className="min-w-0 flex-1 pr-4">
                <span className="text-[10px] text-[#1E4E8C] font-black uppercase tracking-widest block mb-0.5">
                  {subService.parentTitle || "Sub Service"} Details
                </span>
            ...
            <div className="overflow-y-auto flex-1 p-6 space-y-6 [&::-webkit-scrollbar]:w-0.5">
              ...
              <div className="flex items-center justify-between bg-slate-50/60 p-4 rounded-2xl border border-slate-100/50">
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Price</span>
                  <div className="text-[#1E4E8C] text-xl font-black">
                    ৳{Number(subService.price).toLocaleString()}
                  </div>
                </div>
                <div className="w-32">
